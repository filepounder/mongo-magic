...
class MongoQuery {
    ...
    static parseRawQuery(rawQueryString) {
        ...
        if (!checkTypes.object(rawQueryString)) {
            try {
            ...
            try {
                rawQuery = JSON.parse(JSON.stringify(rawQueryString));
            } catch (exp) {
                throw new Error("Invalid Raw Query String")
            }
        }
        ...
        const mergeRawQueryRecursive=(obj, parent, parentKey)=> {
            for (let key in obj) {
                ...
                if (checkTypes.object(obj[key])){
                    mergeRawQueryRecursive(obj[key], obj, key);
                } else if(checkTypes.array(obj[key])){
                    ...
                    if (key === "$date") {
                        ...
                        parent[parentKey] = checkTypes.assigned(obj[key])&&obj[key].toString?obj[key].toString():obj[key];
                        break;
                    }
                }
            }
