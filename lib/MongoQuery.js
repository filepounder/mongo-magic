...
var oDataParser=require('./oDataParser.js');
...
class MongoQuery{
    constructor(query,defaults={}){
        ...
        if (!checkTypes.assigned(query))originalQuery={};

        if (checkTypes.string(query))
            originalQuery=qs.parse(query);
        else
            if (checkTypes.object(query))originalQuery=query;
        else
...
function getTopOrLimit(query){
    ...
    if (limit===NaN){limit=_defaultLimit;}
